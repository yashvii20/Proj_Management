    ...
    <div style={s.app} className='app-shell'>
      ...
      <div style={s.main}>
        ...
        <div style={s.catRow}>
          ...
          {allCategories.map(c => (
            <div key={c.id} style={{ ...s.catChip, ...(selectedCategory === c.id ? s.catChipActive : {}) }} onClick={() => setSelectedCategory(c.id)}>{c.name}</div>
          ))}
        </div>
            ...
            {filtered.map(item => {
              ...
              return (
                <div key={item.id} style={isPdfOrLink ? s.fileRow : s.card} onClick={() => handleOpenDetail(item)}>
                  {isPdfOrLink ? (
                    <>
                      <div style={{ ...s.fileIcon, ...(firstAttach.file_type === 'pdf' ? s.fileIconPdf : s.fileIconLink) }}>
                        {firstAttach.file_type === 'pdf' ? 'PDF' : 'URL'}
                      </div>
            ...
            <form onSubmit={handleAddItem}>
              ...
              <div style={s.field}>
                ...
                <div style={s.uploadRow}>
                  ...
                  <input ref={fileInputRef} type='file' accept='image/*,application/pdf' style={{ display: 'none' }} onChange={e => e.target.files[0] && handleFileUpload(e.target.files[0])} />
                </div>
              ...
              {currentAttachment ? (
                currentAttachment.file_type === 'image' ? (
                  ...
                  <div style={s.linkPlaceholder}>
                    <div style={s.linkPlaceholderText}>{currentAttachment.file_type === 'pdf' ? 'PDF document' : 'Link reference'}</div>
            ...
            <div style={s.detailRight} className='detail-modal-right'>
              <div style={s.detailRightTop}>
                ...
                <div style={s.detailMeta}>
                  <span style={s.catTag}>{selectedItem.moodboard_categories ? selectedItem.moodboard_categories.name : 'Uncategorized'}</span>
...
const s = {
  ...
  catChipActive: { border: '1px solid #352a52', color: '#a78bfa', background: '#1c1730' },
